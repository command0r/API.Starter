...
using API.Starter.Domain.Common.Contracts;
using API.Starter.Infrastructure.Common;
using API.Starter.Infrastructure.Persistence.ConnectionString;
using API.Starter.Infrastructure.Persistence.Context;
using API.Starter.Infrastructure.Persistence.Initialization;
using API.Starter.Infrastructure.Persistence.Repository;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.DependencyInjection;
using Serilog;

namespace API.Starter.Infrastructure.Persistence;
        ...
        switch (dbProvider.ToLowerInvariant())
        {
            case DbProviderKeys.Npgsql:
                ...
                return builder.UseNpgsql(connectionString, e =>
                     e.MigrationsAssembly("Migrations"));

            // Future SQL Server Support - Uncomment when needed
            // case DbProviderKeys.SqlServer:
            //     return builder.UseSqlServer(connectionString, e =>
            //          e.MigrationsAssembly("Migrations.SqlServer"));

            default:
                throw new InvalidOperationException($"DB Provider {dbProvider} is not supported. Currently supports: PostgreSQL");
        }
    }
